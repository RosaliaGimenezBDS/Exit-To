...
%no hemos conseguido insertar una tabla con código, volverlo a intentar si hay tiempo
...
En este estudio, hemos dispuesto de \Sexpr{nrow(datos_panel_filtrados)} sujetos encuestados.
