...
medias_por_region_year <- datos_filtrados %>%
  group_by(REGION) %>%
  summarise(
    MEDIA_FELICIDAD = mean(FELICIDAD, na.rm = TRUE),
    MEDIA_SALUD = mean(SALUD, na.rm = TRUE),
...
datos_tabla$GENERO <- factor(datos_filtrados$GENERO, 
                       levels = c(1, 2), 
                       labels = c("Hombre", "Mujer"))

# Convertir la columna 'rel_laboral' a categórico con etiquetas 'Empleado', 'Autónomo' y 'Negocio familiar'
datos_tabla$REL_LABORAL <- factor(datos_filtrados$REL_LABORAL, 
                            levels = c(1, 2, 3), 
...
datos_tabla$SALUD <- factor(datos_filtrados$SALUD, 
                                  levels = c(1, 2, 3,4,5), 
...
datos_tabla$SEGURIDAD <- factor(datos_filtrados$SEGURIDAD, 
                            levels = c(1, 2, 3,4), 
                            labels = c("Muy seguro", "Seguro", "Inseguro", "Muy inseguro"))

# Los datos de FELICIDAD los dejamos en la escala numérica de '0 extremadamente infeliz a 10 extremadamente feliz'
datos_tabla$FELICIDAD <- factor(datos_filtrados$FELICIDAD, 
...
#Al disponer de muchos datos, el panel tiene dificultades para tratarlos, reduzco el tamaño a los datos con REGION

# Crear SUBCONJUNTOS de datos con nombres significativos con los que trabajar en el PANEL

datos_tabla_region_numericos<- datos_filtrados %>% filter(!is.na(REGION))
